...
export default class SyncService {
  ...
  private targetResourceGroup = process.env["TARGET_RESOURCE_GROUP"];
  ...
  private async getLatestCursor(workspace: Workspace, table: Table) {
    // Check if the table exists before checking records
    const targetTables = this.managementClient.tables.listByWorkspace(this.targetResourceGroup, workspace.name);
    let isTargetTableFound = false;
    for await (const targetTable of targetTables) {
      // Skip tables that don't begin with UC
      if (targetTable.name === table.name) {
        isTargetTableFound = true;
        break;
      }
    }
    if (!isTargetTableFound) {
      return undefined;
    }
    ...
    const kustoQuery = `${table.name} | where isnotempty(${secondaryKeyName})
      | top-nested 1 of TimeGenerated by max(TimeGenerated),
          top-nested of ${secondaryKeyName} by Ignore=max(1)
      | summarize SecondaryKey=max(${secondaryKeyName}) by TimeGenerated`;
  ...
  private async getNextPage(workspace: Workspace, tableName: string, cursor: LogCursor,
    ...
    const kustoQuery = `${tableName} | where ((${timeGeneratedName} == todatetime("${cursor.timeGenerated.toISOString()}") and isnotempty(${secondaryKeyName})`
      ...
      + (excludeFromExistingWorkspace ? ` | join kind=leftanti (workspace("${excludeFromExistingWorkspace}").${tableName}) on AzureADDeviceId, $left.TimeGenerated == $right.TimeGenerated` : "")
